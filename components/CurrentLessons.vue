    ...
    computed: {
        ...
        nearestLessons() {
            ...
            for (let i = 0; i < currentDaySchedule.length; ++i) {
                ...
                const begin = new Date(`${todayKey.split('.').join('-')}T${currentDaySchedule[i].beginLesson}+03:00`);
                const end = new Date(`${todayKey.split('.').join('-')}T${currentDaySchedule[i].endLesson}+03:00`);
